...
class Chromedriver(Utility, OS_type):
    ...
    def evaluate_on_environment(self, os_name, arch_type):
        dir_path = self.get_driver_path("/dependencies/dir_chromedriver")
        if os_name == 'macos' and arch_type == '64':
            ...
            self.log_message("INFO","Downloading the required binary for chromedriver")
            self.driver_downloader(url_builder_mac, dir_path)
            print "\n"
            self.log_message("INFO", "Download completed")
            self.unzip_file('dir_chromedriver/')
        ...
        if os_name == 'linux' and arch_type == '64':
            ...
            self.log_message("INFO","Downloading the required binary for chromedriver")
            self.driver_downloader(url_builder_linux, dir_path)
            print "\n"
            self.log_message("INFO", "Download completed")
            self.unzip_file('dir_chromedriver/')
            self.log_message("INFO","Unarchiving contents completed")
    ...
    def download_driver(self):
        ...
        if os.path.exists(dir_path):
            self.log_message("INFO","chrome driver is already present. To update chromedriver please run `browserium update --driver=chromedriver`")
        else:
    ...
    def update_driver(self):
        self.check_directory_content("/dependencies/dir_chromedriver/chromedriver")
        self.log_message("INFO","Deleting directory contents")
        self.delete_dir_contents('dir_chromedriver/')
        os_name = self.os_name()
        arch_type = str(self.os_architecture())
        self.evaluate_on_environment(os_name, arch_type)
        self.log_message("INFO","chromedriver updated")